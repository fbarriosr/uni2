...
export default function ItineraryClientPage({ salidaId, allActivities }: ItineraryClientPageProps) {
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-4xl">
      ...
      >
        {!isEditing && canEdit && (
          <Button variant="outline" onClick={() => setIsEditing(true)} className="rounded-full h-11 shadow-sm">
            <Pencil className="mr-2 h-4 w-4" /> Editar Itinerario
          </Button>
        )}
        {isEditing && (
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={handleCancel} className="rounded-full h-11 shadow-sm" disabled={isSaving}>
                <X className="mr-2 h-4 w-4" /> Cancelar
            </Button>
            <Button onClick={handleSave} className="rounded-full h-11 shadow-sm" disabled={isSaving}>
        ...
        <TooltipProvider>
            <Tooltip>
                <TooltipTrigger asChild>
                    <Button variant="outline" size="icon" onClick={handleShareToWhatsApp} className="bg-green-100 text-green-700 hover:bg-green-200 border-green-300 rounded-full h-11 w-11 shadow-sm">
